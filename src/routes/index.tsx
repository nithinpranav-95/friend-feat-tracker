...
function RanksView({players,openPlayer}:{players:Player[];openPlayer:(id:string)=>void}) { return <section><p className="font-bold text-primary">ALL GAMES · ALL TIME</p><h2 className="mt-1 font-heading text-4xl font-bold">Leaderboard</h2><div className="mt-6 overflow-hidden rounded-[1.5rem] border border-border bg-card">{players.map((p,i)=><button key={p.id} onClick={()=>openPlayer(p.id)} className="flex w-full items-center gap-4 border-b border-border p-4 text-left last:border-0 hover:bg-secondary"><span className={`grid size-10 place-items-center rounded-xl font-black ${i===0?"bg-primary text-primary-foreground":"bg-secondary"}`}>{i+1}</span><span className="text-3xl">{animals[p.spirit_animal]}</span><div className="flex-1"><p className="font-heading text-lg font-bold">{p.display_name}</p><p className="text-sm text-muted-foreground">{statData[i]?.wins??5} wins · {190-i*22} pts</p></div><Trophy className={i===0?"text-primary":"text-muted-foreground"}/></div>)}</div></section>; }
