...
function GameApp() {
  ...
  return <div className="min-h-screen bg-background pb-24 text-foreground">
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 md:px-7">
      ...
      {tab === "ranks" && <RanksView players={players} openPlayer={setProfileId} />}
...
function RanksView({players,openPlayer}:{players:Player[];openPlayer:(id:string)=>void}) { return <section><p className="font-bold text-primary">ALL GAMES · ALL TIME</p><h2 className="mt-1 font-heading text-4xl font-bold">Leaderboard</h2><div className="mt-6 overflow-hidden rounded-[1.5rem] border border-border bg-card">{players.map((p,i)=><button key={p.id} onClick={()=>openPlayer(p.id)} className="flex w-full items-center gap-4 border-b border-border p-4 text-left last:border-0 hover:bg-secondary"><span className={`grid size-10 place-items-center rounded-xl font-black ${i===0?"bg-primary text-primary-foreground":"bg-secondary"}`}>{i+1}</span><span className="text-3xl">{animals[p.spirit_animal]}</span><div className="flex-1"><p className="font-heading text-lg font-bold">{p.display_name}</p><p className="text-sm text-muted-foreground">{statData[i]?.wins??5} wins · {190-i*22} pts</p></div><Trophy className={i===0?"text-primary":"text-muted-foreground"}/></button>)}</div></section>; }
...
function HistoryView({sessions}:{sessions:PastSession[]}) { const rows = sessions.length ? sessions.map(s=>({key:s.id,g:s.gameName,w:s.results[0]?.name??"—",d:`${s.date} · ${s.rounds} rounds`,s:`${s.results[0]?.score??0} pts`})) : [{key:"a",g:"Uno",w:"Maya",d:"Today · 9 rounds",s:"142 pts"},{key:"b",g:"Poker",w:"Nithin",d:"12 Sep · 6 hands",s:"86 pts"},{key:"c",g:"Sevens",w:"Rita",d:"5 Sep · 4 rounds",s:"18 pts"}]; return <section><p className="font-bold text-primary">THE ARCHIVES</p><h2 className="mt-1 font-heading text-4xl font-bold">Session history</h2><div className="mt-6 space-y-3">{rows.map(x=><div key={x.key} className="flex items-center gap-4 rounded-2xl border border-border bg-card p-4"><span className="grid size-12 place-items-center rounded-xl bg-secondary text-2xl">🏆</span><div className="flex-1"><p className="font-heading text-lg font-bold">{x.g} · {x.w} won</p><p className="text-sm text-muted-foreground">{x.d}</p></div><p className="font-bold text-primary">{x.s}</p></div>)}</div></section>; }
